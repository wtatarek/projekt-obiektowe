...
public abstract class AbstractWorldMap implements IWorldMap, IPositionChangeObserver {
    ...
    public boolean canMoveTo(Vector2d position, Vector2d oldPosition) {
       /* for(Animal i : animals ){
            if(i.getPosition().equals(position)){
                //makeABaby(position,oldPosition);
                return false;
            }
        }*/
    ...
    @Override
    public void run(MoveDirection[] directions, IWorldMap map) {
        MoveDirection[] moves = directions;
        int i = 0;
        for (Animal a : animals) {
            Vector2d v = a.move(moves[i]);
    ...
    public void makeABaby(Animal animal1, Animal animal2) {
        ...
        if (energy1 >= 0.5 && energy2 >= 0.5) {
            ...
            //System.out.println("dokonczyc make a baby");
            //System.out.print("tworzę dziecko z wektorow :"+animal1.getPosition().toString()+"+"+animal2.getPosition().toString()+"= "+x1+ y1);
